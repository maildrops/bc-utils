...
from bcutils.research.config import PROJECT_ROOT, load_instruments, load_roll_rule, selected_symbols
from bcutils.research.continuous import build_continuous_series, save_continuous_result


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build continuous hourly futures series.")
    parser.add_argument("--symbol", required=True, help="Instrument symbol, or ALL")
    parser.add_argument("--roll-days", type=int, default=None, help="Override roll days before expiry")
...
def main() -> None:
    ...
    for symbol in selected_symbols(args.symbol, instruments):
        ...
        if args.dry_run:
            print(f"{symbol}: {len(result.unadjusted)} hourly rows, {len(result.roll_schedule)} rolls")
            continue
