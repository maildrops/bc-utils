...
from bcutils.research.contracts import ContractInfo, infer_contract_info, sorted_contracts
...
def normalize_columns(frame: pd.DataFrame, timezone: str = "UTC") -> pd.DataFrame:
    ...
    return frame[required].dropna(subset=["open", "high", "low", "close"]).sort_values("timestamp")
...
def load_contract_file(path: Path, symbol: str, source: str = "barchart", timezone: str = "UTC") -> pd.DataFrame:
...
def load_all_contracts(
    ...
    if not files:
        raise FileNotFoundError(f"No raw CSV files found in {data_root / 'raw_contracts' / symbol}")
    frames = [load_contract_file(path, symbol, source=source, timezone=timezone) for path in files]
...
def contract_infos(data_root: Path, symbol: str) -> List[ContractInfo]:
    return sorted_contracts(infer_contract_info(path, symbol) for path in raw_contract_files(data_root, symbol))
